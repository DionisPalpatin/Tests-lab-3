from sql_faker.table import Table
from sql_faker.functions import list_to_string, check_type
...
class Database:
    ...
    columns. Every data structure, that is done with generator.sqlfaker_natali starts 
    with a database.

    Currently, generator.sqlfaker_natali only supports mysql/mariadb syntax. The db_type parameter
    is therefore useless at the moment and will be set to mysql by default.
    
    :param db_type: The type of database to export SQL for (generator.sqlfaker_natali currently only supports mysql/mariadb)
